...
def kirim_semua():
    ...
    for k in urls:
        ...
        if temp == 0:
            texec[k] = threading.Thread(target=kirim_gambar, args=(UDP_IP_ADDRESS, 5000, f"{k}.jpg"))
            print('masuk server 1')
            temp = temp + 1
        elif temp == 1:
            print('masuk server 2')
            texec[k] = threading.Thread(target=kirim_gambar, args=(UDP_IP_ADDRESS2, 5000, f"{k}.jpg"))
